...
from metrics import custom_metrics, train_metrics, test_metrics, label_metrics, save_auc
from models import CustomMatrixDataset, MatrixDataset, LossWrapper, NNG
...
def train_test_graph(num_epochs, dimension, train_loader, test_loader, device, group):
    ...
    model = NNG(input_size, dimension[-2], output_size)
    model.to(device)
